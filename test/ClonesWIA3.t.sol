...
contract MockGetArgBytes {
    ...
    fallback() external payable {
        bytes memory arg = LibClonesWIA.getArgBytes3(argOffset, argLen);
...
// ---------------------------------------------------------------------
// Immutable Args Type 3 Gas Tests
// ---------------------------------------------------------------------

contract TestImmutableArgsGas3 is TestImmutableArgsGas {
    function deployMockCloneWIA(
        ...
        return LibClonesWIA.deployCloneWithImmutableArgs3(implementation, "", immutableArgs);
    }
...
contract TestImmutableArgs is Test {
    ...
    function test_getMsgData(bytes memory randomCalldata, bytes memory immutableArgs) public {
        address clone = LibClonesWIA.deployCloneWithImmutableArgs3(address(new MockGetMsgData()), "", immutableArgs);
    ...
    function test_getArgsLen(bytes memory randomCalldata, bytes memory immutableArgs) public {
        address clone = LibClonesWIA.deployCloneWithImmutableArgs3(
